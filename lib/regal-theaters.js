...
// caches it for a week. regmovies.com is Cloudflare-protected
...
// Weekly, not daily. The list changes a few times a year, and it is fetched
// through Camofox -- so a daily expiry put a Camofox round trip (often against
// a cold browser) on the first search of every morning.
const CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1000;
const CACHE_KEY = "regal-theaters";
let memCache = null; // { theaters, fetchedAt }
let inFlight = null;

function getRegalTheaters() {
  if (memCache && Date.now() - memCache.fetchedAt < CACHE_TTL_MS) {
    return Promise.resolve(memCache.theaters);
  }
  // /api/search and /api/search-regal both ask at the same moment; without
  // this each opened its own Camofox tab to fetch the same list.
  if (!inFlight) inFlight = loadTheaters().finally(() => { inFlight = null; });
  return inFlight;
}

async function loadTheaters() {
  const diskData = await readCache(CACHE_KEY, CACHE_TTL_MS);
  if (diskData) {
  ...
  writeCache(CACHE_KEY, theaters, CACHE_TTL_MS);
  return theaters;
}
