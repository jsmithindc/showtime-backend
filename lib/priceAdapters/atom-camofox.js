...
// 4 allows all discovery tabs to run in parallel and price 4 showtimes at once,
// without hitting the ~6-tab overload threshold we saw at the original (unlimited) concurrency.
const camofoxLimit = pLimit(4);
...
async function getAtomCheckoutPricingViaCamofox({ theaterUrl, movieTitle, time12h, format }) {
  ...
  try {
    tabId = await openTab(theaterUrl);
    let discoverySnapshot = (await getSnapshot(tabId)).snapshot;
    let showtimes = parseSnapshotShowtimes(discoverySnapshot, movieTitle);

    // If showtimes are empty the page may have loaded before the XHR completed
    // (networkidle fired before Atom's showtime XHR) or Atom served a partial
    // page under concurrent load. Retry once after a short wait.
    if (showtimes.length === 0) {
      await new Promise((r) => setTimeout(r, 2500));
      discoverySnapshot = (await getSnapshot(tabId)).snapshot;
      showtimes = parseSnapshotShowtimes(discoverySnapshot, movieTitle);
    }
    ...
    if (!target) {
      console.error(`Camofox/Atom: showtime ${time12h} not found in fresh snapshot for "${movieTitle}" (${showtimes.length} showtimes found)`);
