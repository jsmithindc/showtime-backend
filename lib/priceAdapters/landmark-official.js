...
async function getLandmarkPricing(bookingUUID) {
  ...
  const guestTickets = ticketTypes.filter(
    (t) => !t.isMemberTicket && !t.isRedemptionTicket && !t.isLoyaltyCreditTicket && !t.isCardPaymentPromotionTicket && t.price > 0
  );

  // Then drop age- and eligibility-restricted tiers. Two reasons, and the
  // second is the important one:
  //
  //  1. They are noise. Nobody comparing showings wants a senior price they
  //     can't buy listed under every Landmark card.
  //  2. The cheapest surviving ticket becomes THIS SHOWING'S headline price,
  //     and the whole app is a cheapest-ticket comparison ACROSS chains. A
  //     child ticket undercutting the adult one would make a theater look
  //     cheaper than anyone could actually buy it for -- the same failure the
  //     Harkins fee estimator deliberately guesses high to avoid. It happens
  //     not to bite at Greenwood Village (Bargain $15.24 is already below
  //     Child $15.99), which is exactly why it would have gone unnoticed.
  //
  // "Bargain" and "Matinee" are NOT filtered: they are general-admission
  // discounts anyone can buy, so they are legitimately the adult price.
  const NON_ADULT_TICKET = /\b(child|children|kid|kids|youth|teen|senior|student|military|veteran|infant|baby)\b/i;
  let standard = guestTickets.filter((t) => !NON_ADULT_TICKET.test(t.displayName || ""));

  if (standard.length === 0 && guestTickets.length > 0) {
    // Every type looked age-restricted, which almost certainly means the
    // pattern above is wrong for this theater rather than that no adult ticket
    // exists. Some price beats none -- but say so, because the headline number
    // is then not comparable to other chains.
    console.error(
      `Landmark pricing: every ticket type looked age-restricted ` +
      `(${guestTickets.map((t) => t.displayName).join(", ")}) -- falling back to the cheapest of them.`
    );
    standard = guestTickets;
  }
