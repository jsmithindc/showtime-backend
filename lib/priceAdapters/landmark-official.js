 ...
 */
// 10s was too tight in practice. Two things ate it: camofox-regal failing
// first on every call (it can only serve regmovies.com -- it now declines
// these outright), and Bright Data's sync path, which is measured in the tens
// of seconds rather than single digits. The budget expired, this returned
// null, and the whole chain reported "pricing resolved for 0 theater(s)" while
// a working provider was still mid-request.
//
// Landmark pricing runs concurrently with every other chain and its theaters
// run in parallel with each other, so this is added latency only if it is the
// slowest thing in the search.
//
// NOTE: losing the race abandons the result but does NOT cancel the request --
// fetchWithRotation takes no abort signal. A timeout here can therefore still
// spend a provider credit on a response nobody reads. Another reason to make
// the budget generous enough that it rarely fires.
...
async function getLandmarkPricing(bookingUUID) {
  ...
  let res;
  try {
    // needsRender: false — direct HTTP proxy (Bright Data etc.) is more reliable than
    // a browser for a JSON API endpoint. Byparr skips needsRender:false requests.
    // Hard 10s timeout so a slow proxy chain doesn't stall the whole response.
    ({ res } = await Promise.race([
      fetchWithRotation(url, { needsRender: false }),
      new Promise((_, reject) => setTimeout(() => reject(new Error("pricing timeout")), PRICING_TIMEOUT_MS)),
    ]));
  } catch (err) {
    console.error(`Landmark pricing: proxy fetch failed for ${bookingUUID}:`, err.message);
    return null;
  }
  ...
  try {
    ...
    console.error(`Landmark pricing: parse failed for ${bookingUUID}:`, err.message);
    return null;
  }
  ...
  standard.sort((a, b) => a.price - b.price);
  return standard.map((t) => ({
