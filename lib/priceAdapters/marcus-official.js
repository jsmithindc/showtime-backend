...
function getMarcusToken() {
  return process.env.MARCUS_TOKEN || null;
}
...
function marcusHeaders() {
  const token = getMarcusToken();
  return {
    ...
    cinemas.map(async (cinema) => {
      ...
      await Promise.all(
        matchedFilms.map(async (film) => {
          let showtimes;
          try {
            showtimes = await fetchWithTimeout(
          ...
          for (const st of showtimes) {
            if (st.ticketstatus === "Sold Out" || st.seatsavailable === 0) continue;

            // showtime is UTC ISO; convert to local HH:MM using the cinema's tz
            const startTimeRaw = utcIsoToLocalHHMM(st.showtime, cinema.tz);
...
function utcIsoToLocalHHMM(utcIso, tz) {
  try {
