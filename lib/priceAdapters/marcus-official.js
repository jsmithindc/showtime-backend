...
async function getMarcusShowtimesForCinemas({ cinemas, movieTitle, dateISO }) {
  ...
  await Promise.all(
    cinemas.map(async (cinema) => {
      ...
      // Falsy movieTitle means "everything at this cinema" -- the free-time
      // search needs the whole schedule, and this adapter already fetches it
      // in full and filters here. Explicit rather than relying on
      // matchesMovie("", x) happening to return true.
      const matchedFilms = films.filter((f) => !movieTitle || matchesMovie(f.title, movieTitle));
      ...
      await Promise.all(
        matchedFilms.map(async (film) => {
          ...
          for (const st of showtimes) {
            ...
            // SDate requires dashes (YYYY-MM-DD) and STime requires colon (HH:MM)
            const bookingLink = `https://ticketing.marcustheatres.com/seat/layout?Siteid=${cinema.cinemaid}&FeatureCode=${film.filmid}&SDate=${dateISO}&STime=${startTimeRaw}&Perf=${st.sessionid}`;

            allEntries.push({
              // Needed by the free-time search, which asks for every film at a
              // cinema rather than one known title.
