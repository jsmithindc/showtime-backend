...
// Auth: Bearer JWT token, configured via MARCUS_TOKEN env var.
// The token is a CINEMAAPP-role app credential (email: website@injin.com).
// Its exp claim says 30 min, but the server appears to accept it for much longer;
// nonetheless, operators should refresh it if requests start returning 401.
// Flow:
...
// Auto-refresh token: extracted from the ticketing site's JS bundle.
// Falls back to MARCUS_TOKEN env var if set and not expired.
...
const TOKEN_REFRESH_BUFFER_SEC = 120; // refresh when < 2 min from expiry
const BUNDLE_FETCH_TIMEOUT_MS = 20000;

async function fetchMarcusTokenFromBundle() {
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), BUNDLE_FETCH_TIMEOUT_MS);
  let html;
  try {
    const r = await fetch("https://ticketing.marcustheatres.com/", { signal: ctrl.signal });
    html = await r.text();
  } finally {
    clearTimeout(t);
  }
  ...
  // Also check for any inline JWT in the HTML itself (some SPAs embed env at build time)
  const inlineMatches = html.match(JWT_RE) || [];
  for (const token of inlineMatches) {
    const result = tryExtractCinemaAppToken(token);
    if (result) return result;
  }

  for (const src of scripts) {
    const url = `https://ticketing.marcustheatres.com${src}`;
    const ctrl2 = new AbortController();
    const t2 = setTimeout(() => ctrl2.abort(), BUNDLE_FETCH_TIMEOUT_MS);
    let js;
    try {
      const r2 = await fetch(url, { signal: ctrl2.signal });
      js = await r2.text();
    } catch {
      continue;
    } finally {
  ...
  throw new Error("Marcus: could not find CINEMAAPP token in ticketing.marcustheatres.com JS bundles");
}

function tryExtractCinemaAppToken(token) {
  try {
    // Base64url → base64 padding fix
    const payloadB64 = token.split(".")[1].replace(/-/g, "+").replace(/_/g, "/");
    const payload = JSON.parse(Buffer.from(payloadB64, "base64").toString("utf8"));
    if (payload.role === "CINEMAAPP" || payload.email === "website@injin.com") {
...
async function getMarcusToken(forceRefresh = false) {
  ...
  console.error("Marcus: fetching token from ticketing.marcustheatres.com JS bundle...");
  const { token, exp } = await fetchMarcusTokenFromBundle();
  _cachedToken = token;
  _tokenExp = exp;
  const expStr = exp ? new Date(exp * 1000).toISOString() : "unknown";
  console.error(`Marcus: token refreshed from bundle (exp ${expStr})`);
  return _cachedToken;
}
