...
async function getMarcusShowtimesForCinemas({ cinemas, movieTitle, dateISO }) {
  ...
  const theaterSummary = theaters.map((c) => {
    const n = allEntries.filter((e) => e.cinema.id === c.id).length;
    return `${c.name} (${n} session(s))`;
  }).join(", ");
  console.error(`Marcus: fetched ${allEntries.length} session(s) matching "${movieTitle}" on ${dateISO} across ${theaters.length} cinema(s): ${theaterSummary}`);
