...
const camofox = require("../proxyProviders/camofox-cinemawest");
...
// real and built (see getTicketPricing / getShowtimesForSite below). The
// Bearer token is anonymous, lasts 12 hours, and is read fresh from the
// theater page's HTML (see getFreshToken). All three requests go through
// Camofox, because the site refuses Render's IP (see cwFetch).
...
// Both hosts sit behind Cloudflare, and the site 403s Render's IP outright, so
// requests go through Camofox when it is configured and direct otherwise.
// In-page, only the headers the site's own JS sends: User-Agent/Origin/Referer
// are ignored or would widen the CORS preflight the API has no reason to allow.
function cwFetch(url, bearerToken) {
  if (camofox.isConfigured() && camofox.supports(url)) {
    return camofox.fetchViaProvider(
      url,
      bearerToken ? { headers: { Accept: "application/json", Authorization: `Bearer ${bearerToken}` } } : {}
    );
  }
  return fetch(url, {
    headers: bearerToken ? { ...HEADERS, Authorization: `Bearer ${bearerToken}` } : HEADERS,
  });
}
...
// A bare fetch from Render is refused: 403 on every search. From a residential
// browser the page is 200 with the token present (see cwFetch).
async function getFreshToken(sitePath) {
  const url = `https://web.cinemawest.com/sites/${sitePath}`;
  const res = await cwFetch(url);
  if (!res.ok) {
...
async function getShowtimesForSite({ siteId, bearerToken }) {
  ...
  const res = await cwFetch(url, bearerToken);
