...
const camofoxCinemark = require("../proxyProviders/camofox-cinemark");
...
//
// Pacing alone did NOT fix this, and the evidence says why: the failure moved
// from HTTP 429 (a rate limit -- 14 of 18 showtimes still priced) to a
// Cloudflare challenge served as HTTP 200 on the FIRST request of the search.
// That is an IP-reputation verdict on cinemark.com, not a verdict on this
// search's request rate, so spacing requests further apart cannot clear it --
// it just spreads twenty guaranteed failures over a longer window.
//
// So once a challenge is seen, stop asking. Every subsequent request in that
// state is certain to fail, costs a round trip, and adds load to an IP that is
// already flagged. One real failure replaces twenty.
const CHALLENGE_COOLDOWN_MS = Number(process.env.CINEMARK_CHALLENGE_COOLDOWN_MS) || 120000;
let challengedUntil = 0;

function challengeError(message) {
  const err = new Error(message);
  err.cinemarkRateLimited = true;   // lets the caller log it without a stack
  return err;
}

function tripBreaker() {
  const wasOpen = challengedUntil > Date.now();
  challengedUntil = Date.now() + CHALLENGE_COOLDOWN_MS;
  if (!wasOpen) {
    console.error(
      `Cinemark: Cloudflare challenge -- skipping Cinemark pricing for ${Math.round(CHALLENGE_COOLDOWN_MS / 1000)}s. ` +
      `This is an IP-reputation block, not a rate this app can pace around.`
    );
  }
}
...
// Camofox when it is configured, a direct fetch otherwise.
//
// This is the actual fix for the Cloudflare block: cinemark.com was the only
// chain still requested from Render's datacenter IP, and that IP is now
// challenged on sight. Camofox runs on a residential connection, which is why
// Regal works from the same host.
//
// Two deliberate differences on the Camofox path:
//   - No pacing. The gap exists to avoid tripping a rate limit on a shared
//     datacenter IP; more importantly, serializing would DEFEAT the factory's
//     GET batching, which needs requests to overlap in a 30ms window to
//     coalesce them into one browser round trip.
//   - No headers. isBatchable() only batches header-free GETs, and a real
//     browser supplies better headers than this module's constants anyway.
function viaCamofox(url) {
  return camofoxCinemark.isConfigured() && camofoxCinemark.supports(url);
}

function siteFetch(url) {
  if (viaCamofox(url)) return camofoxCinemark.fetchViaProvider(url, {});
...
async function getTicketPricing({ theaterId, showtimeId, cinemarkMovieId, showtimeISO }) {
  ...
  if (Date.now() < challengedUntil) {
    throw challengeError(
      `Cinemark is challenging this IP -- skipped without a request (${Math.round((challengedUntil - Date.now()) / 1000)}s left).`
    );
  }

  // The Camofox provider detects a challenge ITSELF and throws rather than
  // returning a response, so the isChallengePage checks below never ran and the
  // breaker never tripped -- every one of ~19 showtimes kept asking a host that
  // had already said no. Recognise a thrown challenge as the same verdict.
  let res;
  try {
    res = await siteFetch(url);
  } catch (err) {
    if (/cloudflare challenge/i.test(err.message)) {
      tripBreaker();
      throw challengeError(`Cinemark rate-limited this request -- ${err.message}`);
    }
    throw err;
  }
