...
//
// indexOf rather than a regex, for robustness on very large pages. This is
// NOT the fix for the "Maximum call stack size exceeded" seen on every
// Cinemark pricing call in production -- the greedy-backtracking theory that
// prompted this rewrite was tested directly and disproven: the old
// /var\s+{varName}\s*=\s*'([^']+)'/ returns null cleanly against a 1MB
// unterminated payload rather than throwing. That failure is still
// undiagnosed; see the err.stack logging at the Cinemark call site.
// Keeping this version anyway because scanning for the delimiters is simpler
// and has no pathological input, but don't read it as a solved problem.
