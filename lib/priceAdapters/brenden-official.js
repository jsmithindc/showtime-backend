...
const SHOWING_FIELDS = `id time priceCardId showingBadgeIds seatsRemainingByTicketTypeId displayMetaData
                        movie { name urlSlug }`;

function normaliseShowing(s) {
  ...
  return {
    ...
    // seatsRemainingByTicketTypeId is keyed by every ticket type this showing
    // sells -- the only signal for which types are even applicable.
...
/**
 * One showing by id, independent of any date.
 *
 * The date-scoped listing is the fast path, but it CANNOT be relied on alone:
 * /{site}/showtimes always returns the venue's own "today" and ignores the
 * date asked for, while showingsForDate honours it. Render runs in UTC, so
 * from 17:00 Pacific onward the server's date is already tomorrow at a Nevada
 * venue -- the showings come from one day and the price context from the next,
 * and nothing matches. A showing carries its own price card and ticket types
 * regardless of which day you ask about, so this sidesteps the mismatch
 * entirely rather than trying to guess the venue's local date.
 */
async function getShowingById(site, showingId) {
  const data = await gql(
    `query ($id: ID!) { showing(id: $id) { ${SHOWING_FIELDS} } }`,
    { id: showingId },
    siteIdFor(site)
  );
  return data && data.showing ? normaliseShowing(data.showing) : null;
}
...
async function getShowingContext(site, dateISO) {
  ...
  for (const s of (data.showingsForDate && data.showingsForDate.data) || []) {
    out[String(s.id)] = normaliseShowing(s);
  }
  writeCache(key, out, ttl);
  return out;
}
...
async function getTicketPricing({ site, showingId, dateISO }) {
  ...
  // The day listing is the cheap path -- one request covers every showing at
  // the site. When it misses (see getShowingById for why that is routine, not
  // exceptional), ask about the one showing directly rather than failing.
  const ctx = showings[String(showingId)] || await getShowingById(site, showingId);
  if (!ctx) return null;
...
module.exports = {
  ...
  getTicketPricing, getPricingConfig, getShowingContext, getShowingById,
  ticketRowsFor, feesForTicket, pickAdult,
  SITE_IDS,
};
