...
async function getShowtimesForTheater({ cinemaCode, dateISO, costTracker }) {
  ...
  // Temporary diagnostic: log Auditorium + PerformanceAttributes for the
  // first few performances so real format values can be confirmed.
  const sample = performances.slice(0, 5);
  console.error(`Regal format sample: ${JSON.stringify(sample.map((p) => ({ screenName: p.screenName, attrNames: p.attrNames })))}`);
  return performances;
}
