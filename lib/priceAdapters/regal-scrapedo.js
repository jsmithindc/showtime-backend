...
async function getShowtimesForTheater({ cinemaCode, dateISO, costTracker }) {
  ...
  // Log all distinct keys seen on Performance objects so format field
  // names can be identified from real responses without guessing.
  const perfKeysSeen = new Set();
  const performances = [];
  for (const film of films) {
    for (const perf of film.Performances || []) {
      if (!perf.PerformanceId || !perf.CalendarShowTime) continue;
      for (const k of Object.keys(perf)) perfKeysSeen.add(k);

      // Attempt to extract a human-readable format/screen type from
      // likely Vista field names. Field names are unconfirmed -- this
      // captures whatever is present and falls back to null so the
      // caller can default to "Standard".
      const attrs = perf.Attributes || perf.ExperienceAttributes || perf.ScreenAttributes || [];
      const attrNames = Array.isArray(attrs)
        ? attrs.map((a) => a.Description || a.Name || a.AttributeDescription || String(a)).filter(Boolean)
        : [];
      const screenType = perf.ScreenType || perf.ExperienceType || perf.FormatType || null;
      const screenName = perf.ScreenName || perf.CinemaName || null;
  ...
  if (perfKeysSeen.size > 0) {
    console.error(`Regal listing: Performance object keys seen: ${[...perfKeysSeen].join(", ")}`);
  }
  return performances;
}
...
function deriveRegalFormat(perf) {
  ...
  // Check explicit screen-type field first.
  if (perf.screenType && PREMIUM.test(perf.screenType)) return perf.screenType;
  // Then attribute list.
  const premiumAttr = (perf.attrNames || []).find((n) => PREMIUM.test(n));
  if (premiumAttr) return premiumAttr;
  // Then screen name (e.g. "RPX Theatre 1").
...
async function createOrderAndGetTicketsWithRetry({ cinemaCode, performanceId, costTracker }) {
  ...
  for (const perf of matching) {
    try {
      ...
      results.push({ time: perf.showTime, price: null, movieId: perf.movieId, performanceId: perf.performanceId, format: deriveRegalFormat(perf) });
    }
  }
