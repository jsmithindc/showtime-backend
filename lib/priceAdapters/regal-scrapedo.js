...
async function getPricedShowtimes({ cinemaCode, movieTitle, dateISO, candidateMinutes, preDiscoveredPerformances, costTracker: externalCostTracker, onResult, isAborted }) {
...
//
// The cart is minted LAZILY on first use rather than eagerly per theater,
// so a theater whose showtimes all get filtered out never opens one at all.
  ...
  return {
    ...
    get() {
      if (!cartPromise) {
        generation++;
        console.error(
          `Regal [${cinemaCode}]: opening cart (generation ${generation}) -- shared by every performance at this theater`
        );
...
async function createOrderAndGetTicketsWithRetry({ cinemaCode, performanceId, cart, costTracker }) {
  ...
  const results = [];
  // One cart for this whole theater, shared by every performance below and
  // minted lazily on the first one that actually needs it. See the comment
  // on makeCartProvider for the live evidence that this is safe.
  const cart = makeCartProvider({ cinemaCode, costTracker });
...
module.exports = { getPricedShowtimes, getShowtimesForTheater, getCachedShowtimesForTheater, getRuntimeForRegalMovie };
