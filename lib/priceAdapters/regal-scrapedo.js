...
async function getShowtimesForTheater({ cinemaCode, dateISO, costTracker }) {
  ...
  writeDiskCache(cacheKey, performances);
  return performances;
}
...
async function createOrderSession({ cinemaCode, costTracker }) {
  ...
  // needsRender: true routes through byparr first. FlareSolverr's request.post
  // sends postData as a raw string; whether Regal receives it as JSON depends on
  // byparr's browser behavior. If byparr returns HTML (wrong content-type), it
  // throws and falls through to Bright Data async. Worth trying since byparr is
  // free/fast and Bright Data async has been consistently timing out.
...
async function getTicketsForPerformance({ cinemaCode, performanceId, cartId, costTracker }) {
  ...
  const { res, provider, creditsUsed } = await fetchWithRotation(ticketsUrl, { needsRender: true });
  costTracker.total += creditsUsed;
  costTracker.byProvider[provider] = (costTracker.byProvider[provider] || 0) + creditsUsed;
...
async function getPricedShowtimes({ cinemaCode, movieTitle, dateISO, candidateMinutes, preDiscoveredPerformances, costTracker: externalCostTracker, onResult }) {
...
async function createOrderAndGetTicketsWithRetry({ cinemaCode, performanceId, costTracker }) {
  ...
  const results = [];
  // Price all performances in parallel -- createOrder uses Bright Data's
  // async poll (20s+ wait) so sequential would multiply that by perf count.
...
module.exports = { getPricedShowtimes, getShowtimesForTheater, getCachedShowtimesForTheater };
