...
function extractJson(text) {
  ...
  try {
    ...
    throw new Error(
      `Couldn't find JSON in the response, either <pre>-wrapped or raw. First 200 chars: ${text.slice(0, 200)}`
    );
  }
}
...
async function getShowtimesForTheater({ cinemaCode, dateISO, costTracker }) {
  ...
  if (!res.ok) {
    throw new Error(`${provider} listing request failed: ${res.status} ${res.statusText}`);
  }
...
async function createOrderSession({ cinemaCode, costTracker }) {
  ...
  if (!res.ok) {
    throw new Error(`${provider} createOrder failed: ${res.status} ${res.statusText}`);
  }

  const json = await res.json();
  const cartId = json?.order?.userSessionId;
  if (!cartId) {
    throw new Error(`createOrder response missing order.userSessionId: ${JSON.stringify(json).slice(0, 200)}`);
  }
  return cartId;
}
...
async function getTicketsForPerformance({ cinemaCode, performanceId, cartId, costTracker }) {
  ...
  if (!res.ok) {
    throw new Error(`${provider} getTicketsForSession failed: ${res.status} ${res.statusText}`);
  }
