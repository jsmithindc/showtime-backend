...
// Search Atom Tickets for a Regal theater by name and return its venue ID + slug.
// Returns null if the theater is not listed on Atom.
// Result is cached in-memory (per process) and on disk (survives restarts).
async function lookupAtomVenue(theaterName) {
  ...
  // Theater links: href="/theaters/regal-delta-shores/47446"
  const linkMatches = [...html.matchAll(/href="\/theaters\/([^/"]+)\/(\d+)"/g)];
  const nameCore = normName.replace("regal", "").replace(/cinemas?/, "").replace(/theatres?/, "").trim();

  let result = null;
  for (const [, slug, venueId] of linkMatches) {
    if (!slug.includes("regal")) continue;
    const slugCore = norm(slug.replace(/-/g, " ")).replace("regal", "").trim();
    // Accept if at least 4 chars of the core name overlap
