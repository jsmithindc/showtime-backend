...
// Cache names become filenames. Keys like "spidermanbrandnewday::2026" are
// legal on Linux but not everywhere, and a name containing a slash would
// escape the cache directory entirely. The allowed set deliberately keeps
// . , - + so existing keys (e.g. "drive-times-39.7392,-104.9849") are
// unchanged and don't invalidate themselves on upgrade.
...
function readDiskCache(name, ttlMs) {
  try {
    const file = path.join(CACHE_DIR, `${safeFileName(name)}.json`);
...
async function readCache(name, ttlMs) {
  ...
  try {
    ...
    // This line is the ONLY place the shared tier is observable. A local hit
    // and a Redis hit used to log identically at the call site, so there was
    // no way to tell whether Upstash was earning anything -- or whether its
    // credentials had quietly stopped working. Reaching here means the local
    // file missed (a fresh container, typically just after a deploy) and Redis
    // answered instead of the caller paying for a refetch. Local hits stay
    // silent: callers already log those, and this fires at most once per key
    // per container.
    const ageMin = Math.round((Date.now() - parsed.fetchedAt) / 60000);
    console.error(`Shared cache HIT: "${name}" came from Redis (local file missed), written ${ageMin}min ago -- refetch avoided.`);
    return parsed.data;
  } catch (err) {
