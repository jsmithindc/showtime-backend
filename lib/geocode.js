...
// Fallback geocoder for when Nominatim 429s (common from Render's shared
// datacenter IP range, which gets hammered by many apps using Nominatim).
// Photon is a separate OpenStreetMap-based geocoder run by Komoot -- same
// underlying data, different instance, different rate-limit pool.
...
async function _doGeocode(query) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      if (isRateLimited) {
        // Nominatim is rate-limiting this IP -- fall back to Photon
        // immediately rather than waiting and retrying the same endpoint.
        console.error(`Geocoding "${query}": Nominatim 429, falling back to Photon`);
        try {
          const result = await geocodeViaPhoton(query);
          cache.set(query, result);
          return result;
        } catch (photonErr) {
          throw new Error(`Nominatim rate-limited and Photon also failed: ${photonErr.message}`);
        }
      }
