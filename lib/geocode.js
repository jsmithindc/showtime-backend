...
async function _doGeocode(query) {
  ...
  const fromDisk = await readCache(diskKey, GEOCODE_DISK_TTL_MS);
  ...
  if (process.env.GEOAPIFY_API_KEY) {
    try {
      ...
      writeCache(diskKey, result, GEOCODE_DISK_TTL_MS);
      return result;
    } catch (geoErr) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    try {
      ...
      writeCache(diskKey, resolved, GEOCODE_DISK_TTL_MS);
      return resolved;
    } catch (err) {
      lastErr = err;
