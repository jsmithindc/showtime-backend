...
async function getCinemarkMovieIdForTitle(slug, movieTitle) {
  ...
  const titleIndex = html.toLowerCase().indexOf(htmlEncodedTitle.toLowerCase());
  if (titleIndex === -1) return null;

  const searchWindow = html.slice(titleIndex, titleIndex + TITLE_TO_ID_SEARCH_WINDOW);
  // Two formats seen in the wild:
  //   URL param:  CinemarkMovieId=110641  (TicketSeatMap links)
  //   JSON attr:  "cinemarkMovieId": 110641  (data-json-model blobs, HTML-entity-encoded)
  const match = /CinemarkMovieId=(\d+)/i.exec(searchWindow)
    || /cinemarkMovieId&quot;:\s*(\d+)/i.exec(searchWindow)
    || /cinemarkMovieId":\s*(\d+)/i.exec(searchWindow);
  return match ? match[1] : null;
}

// Every distinct CinemarkMovieId near ANY occurrence of the title, in page
// order. Cinemark lists a film's formats as separate movie entries -- the
// standard run and the IMAX 70mm run of the same film are different ids -- so
// getCinemarkMovieIdForTitle above, which takes the first hit, always returns
// whichever Cinemark happens to list first. A caller that needs a particular
// format has to try each and check what comes back.
async function getCinemarkMovieIdsForTitle(slug, movieTitle) {
  const html = await getRawHtml(slug);
  const needle = movieTitle.replace(/&/g, "&amp;").toLowerCase();
  ...
  for (;;) {
    ...
    const searchWindow = html.slice(at, at + TITLE_TO_ID_SEARCH_WINDOW);
    const match = /CinemarkMovieId=(\d+)/i.exec(searchWindow)
      || /cinemarkMovieId&quot;:\s*(\d+)/i.exec(searchWindow)
      || /cinemarkMovieId":\s*(\d+)/i.exec(searchWindow);
...
module.exports = { getRuntimeForMovieAtTheater, getCinemarkMovieIdForTitle, getCinemarkMovieIdsForTitle };
