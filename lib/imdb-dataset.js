...
// Batches concurrent callers into ONE refresh. A search resolves its titles in
// parallel, so without the debounce the first to arrive would start a run that
// excluded all its siblings -- and they'd each wait for a run that couldn't
// answer them. The short delay lets the whole search register first.
...
//
// Originally this was fire-and-forget, on the assumption that a search must
// never wait on an 8 MB download. Measured, that assumption was wrong: a full
// read for modern IDs (which sit late in the sorted file, so the early exit
// barely helps) takes 0.6s at 14 MB heap. Making the caller wait that once --
// against a search already taking ~20s, and only when something untracked
// appears -- is plainly better than showing a knowingly-stale number and
// telling someone to search again.
//
// Bounded anyway: if the download stalls, the timeout wins and the caller falls
// back to OMDb's figure rather than the search hanging.
async function ensureRating(tconst, timeoutMs = 5000) {
  if (!tconst) return null;
  const known = getRating(tconst);
  if (known != null) return known;

  noteInterest(tconst);
  await Promise.race([scheduleRefresh(), sleep(timeoutMs)]);
  return getRating(tconst);
}
