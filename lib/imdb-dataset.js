...
const { readDiskCache, writeCache, readCache } = require("./disk-cache");
...
// load() is synchronous and called from sync paths (noteInterest/getRating),
// so it can only see the local file. This pulls the shared copy once per
// container, from the one caller that is already async -- after a redeploy it
// restores ratings already resolved elsewhere instead of re-deriving them.
let sharedWarmed = false;
async function warmFromSharedCache() {
  if (sharedWarmed) return;
  sharedWarmed = true;
  try {
    const shared = await readCache(CACHE_KEY, REFRESH_AFTER_MS * 100);
    if (shared && shared.ratings) {
      // Local wins on conflict: it is at least as fresh, and merging this way
      // means a shared entry can only ADD ratings, never revert one.
      ratingsById = { ...shared.ratings, ...(ratingsById || {}) };
      lastRefreshAt = Math.max(lastRefreshAt || 0, shared.fetchedAt || 0);
    }
  } catch {
    // shared tier is best-effort
  }
}

async function refresh({ full = false } = {}) {
  const map = load();
  await warmFromSharedCache();
  // Recomputed AFTER the warm-up: anything the shared tier just supplied is
  // already known and must not be looked up again.
  //
  // `full` re-reads every rating already held, which is what the daily
  // staleness pass wants. Everything else needs only the IDs not yet known --
  // and asking for the known ones too was actively expensive, because the early
  // exit below can fire only once EVERY wanted id has been seen. One unknown
  // title among 300 known ones therefore guaranteed a scan to the end of the
  // file rather than a stop at that title's row.
  const known = ratingsById || map;
  const wanted = full
    ? new Set([...Object.keys(known), ...interestedIds])
    : new Set([...interestedIds].filter((id) => known[id] === undefined));
  ...
  ratingsById = { ...(ratingsById || map), ...next };
  ...
  writeCache(CACHE_KEY, { ratings: ratingsById, fetchedAt: lastRefreshAt }, REFRESH_AFTER_MS * 100);
  console.error(
...
function refreshInBackground() {
  ...
  refreshPromise = (async () => {
    load();
    // Warm from the shared tier BEFORE testing staleness. lastRefreshAt is
    // restored by load() from readDiskCache, which is LOCAL-ONLY -- so on a
    // fresh container it is always 0, every deploy looked stale, and the whole
    // ~8MB export was re-downloaded and re-scanned even when Redis already
    // held a current subset. Warming first means a deploy costs one Redis GET.
    await warmFromSharedCache();
    const stale = Date.now() - lastRefreshAt > REFRESH_AFTER_MS;
    const hasUnknown = [...interestedIds].some((id) => (ratingsById || {})[id] === undefined);
    if (!stale && !hasUnknown) return;
    await refresh({ full: stale });
  })()
