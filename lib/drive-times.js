...
// Geoapify accepts far more than this per call, but each request is billed by
// source-target pair either way, so chunking costs nothing and keeps any one
// failure from losing the whole set.
  ...
  const work = (async () => {
    ...
    for (let start = 0; start < entries.length; start += MAX_TARGETS_PER_CALL) {
      ...
      try {
        ...
        // Leave this chunk null and keep going -- a later chunk may succeed,
        // and every null simply means "caller keeps its estimate".
