...
// Firecrawl rate-limits by REQUESTS PER MINUTE, not by quota -- confirmed with
// over 1,000 credits remaining and 38 used when three venues still got 429s.
// So the fix is pacing, not budget.
//
// A 1.2s gap was not enough: serialised requests still completed roughly every
// 4s including fetch time, which is ~14/min. 6s holds it near 10/min. A full
// twelve-venue sweep therefore takes over a minute, which is fine because
// results are cached to midnight and the 200-mile default means a normal
// lookup touches one or two venues, not twelve.
const FETCH_GAP_MS = 6000;
// How many times a 429 is retried before the venue is reported unchecked.
...
async function fetchViaFirecrawl(url, attempt = 0) {
  ...
  // 429 is pacing, not a refusal of the page, so back off and try again rather
  // than telling the user we can't check a theater. Firecrawl sends
  // Retry-After when it wants a specific wait; honour it instead of guessing,
  // and fall back to a widening delay when it doesn't.
  if (res.status === 429 && attempt < RATE_LIMIT_RETRIES) {
    const headerWait = Number(res.headers.get("retry-after")) * 1000;
    const wait = Number.isFinite(headerWait) && headerWait > 0
      ? Math.min(headerWait, 60000)
      : 8000 * (attempt + 1);
    console.error(`Atom 70mm: rate limited, waiting ${Math.round(wait / 1000)}s (attempt ${attempt + 1}/${RATE_LIMIT_RETRIES})`);
    await new Promise((r) => setTimeout(r, wait));
    return fetchViaFirecrawl(url, attempt + 1);
  }
 ...
 *
 * Returns { rows, error }. Never throws -- a missing confirmation must not take
 * down the venue list it was meant to enrich -- but the REASON travels with the
 * failure, because "we couldn't reach Atom" and "Atom says nothing is on"
 * otherwise render identically as an absent venue, and only one of those is
 * worth the user retrying.
 */
async function getAtomShowings({ atomSlug, dateISO }) {
  if (!atomSlug) return { rows: null, error: "no Atom page mapped for this venue" };
  const key = `${CACHE_PREFIX}-${atomSlug.replace(/\//g, "-")}-${dateISO}`;
  const ttl = msUntilMidnight();

  // Cache BEFORE the credential check: a page already fetched today is just
  // data, and needs no key to re-read. Checking config first made a
  // missing/exhausted key look like a total outage even for venues we had
  // already answered for minutes earlier.
  try {
    const cached = await readCache(key, ttl);
    if (cached) return { rows: cached, error: null };
  } catch { /* fall through to a fetch */ }

  if (!isConfigured()) return { rows: null, error: "FIRECRAWL_API_KEY not set" };
  ...
  try {
    ...
    // Rate limiting is temporary and worth retrying; anything else probably
    // isn't. The distinction reaches the user, so say which this was.
    const rateLimited = /\b429\b/.test(err.message);
    return { rows: null, error: err.message, retryable: rateLimited };
  }
}
