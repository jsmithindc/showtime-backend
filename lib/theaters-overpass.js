...
async function findNearbyTheaters({ lat, lng, radiusMeters }) {
  const theaters = await _findNearbyTheatersOverpass({ lat, lng, radiusMeters });

  if (theaters.length > 0) return theaters;

  // Overpass returned 0 results -- common from datacenter IPs that Overpass
  // silently blocks. Try Geoapify as a fallback if configured.
  if (process.env.GEOAPIFY_API_KEY) {
    console.error("Overpass returned 0 theaters -- trying Geoapify fallback.");
    try {
      const geo = await findNearbyTheatersGeoapify({ lat, lng, radiusMeters });
      if (geo && geo.length > 0) return geo;
      console.error("Geoapify also returned 0 theaters.");
    } catch (err) {
      console.error("Geoapify fallback failed:", err.message);
    }
  }

  return theaters; // empty
}
