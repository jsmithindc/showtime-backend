...
async function _findNearbyTheatersOverpass({ lat, lng, radiusMeters }) {
  ...
  // Same length guard as the wrapper -- an entry written before that rule
  // existed could still hold [].
  if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS && cached.theaters?.length > 0) {
  ...
  for (const endpoint of OVERPASS_ENDPOINTS) {
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        // NEVER cache an empty result. Overpass answers a rate-limited or
        // throttled request with an empty elements array rather than an error
        // (documented behaviour, and the reason Geoapify became primary), so
        // caching [] pins a transient failure for the full 24h TTL. Worse
        // since the cache moved up into findNearbyTheaters: a poisoned entry
        // now short-circuits Geoapify too, turning one bad Overpass moment
        // into a day of empty searches. Caught by a real window-search
        // returning "no theaters" against a cache file containing [].
  ...
  if (cached && cached.theaters?.length > 0) {
    console.error(
...
async function findNearbyTheaters({ lat, lng, radiusMeters }) {
  ...
  // Length check as well as freshness: an empty cached list is never a real
  // answer worth serving, and treating it as a hit would skip every provider.
  if (cached && Date.now() - cached.fetchedAt < CACHE_TTL_MS && cached.theaters?.length > 0) {
    return cached.theaters;
  }
