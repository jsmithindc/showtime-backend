...
async function getRatings(rawTitle, yearHint) {
  ...
  const promise = (async () => {
    ...
    if (shared !== null && shared !== undefined) {
      ...
      console.error(`Ratings disk/redis cache hit for "${title}" (value: ${shared.value == null ? "null" : "present"})`);
      return shared.value ?? null;
    }
    console.error(`Ratings cache miss for "${title}" -- calling ${mdblist.isConfigured() ? "MDBList" : "OMDb"}`);
    // MDBList when configured; OMDb+RT scraper as fallback.
