...
// Critic/audience scores for a film.
//
// Primary source: MDBList (MDBLIST_API_KEY) — aggregates IMDb, RT, and
// Metacritic in two API calls (search + media info). Cleaner than OMDb: looked
// up by IMDb ID so same-titled films across years never collide.
//
// Fallback: OMDb (OMDB_API_KEY) + optional RT scraper (ENABLE_RT_SCRAPE).
// OMDb's RT data is stale for new releases, which is why the RT scraper exists
// alongside it. MDBList's aggregation replaces both.
//
// Gated like every other adapter -- with no key set, getRatings returns null
// and callers carry on without scores rather than failing.
...
function isConfigured() {
  return mdblist.isConfigured() || (!!process.env.OMDB_API_KEY && !keyRejected);
}
...
async function getRatings(rawTitle, yearHint) {
  ...
  const promise = (async () => {
    ...
    // MDBList when configured; OMDb+RT scraper when it isn't, or when MDBList
    // has no answer (no match, no usable ID, no scores).
    const mdblistResult = mdblist.isConfigured()
      ? await mdblist.fetchRatings(normalized, year).catch((err) => {
          console.error(`MDBList lookup failed for "${normalized}":`, err.message);
          return null;
        })
      : null;
    // MDBList's scores, but the poster is the page header, so borrow OMDb's
    // when MDBList doesn't carry one.
    if (mdblistResult && !mdblistResult.poster && process.env.OMDB_API_KEY && !keyRejected) {
      const omdb = await fetchRatings(normalized, year).catch(() => null);
      if (omdb?.poster) mdblistResult.poster = omdb.poster;
    }
    const value = mdblistResult !== null ? mdblistResult
      : await Promise.all([
