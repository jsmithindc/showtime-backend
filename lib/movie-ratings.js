...
async function fetchRatings(title, yearHint) {
  ...
  if (imdbId) imdbDataset.noteInterest(imdbId);
  const fromDataset = imdbId ? imdbDataset.getRating(imdbId) : null;
  ...
  if (ratings.imdb == null && ratings.rottenTomatoes == null && ratings.metacritic == null) {
    console.error(`Ratings for "${title}": matched "${ratings.matchedTitle}" (${ratings.year}) but it carries no scores.`);
    return null;
  }

  // Which sources actually came back. OMDb's critic aggregates lag well behind
  // IMDb for new releases -- most films in theatres now have an IMDb score and
  // no RT or Metacritic entry yet. Logged once per title (results are cached)
  // so a missing score is visibly OMDb's coverage rather than a guess about
  // whether parsing broke. Also prints the matched title and year, which is how
  // a wrong-film match would show itself.
  const have = [
  ...
  console.error(
    `Ratings for "${title}" -> matched "${ratings.matchedTitle}" (${ratings.year}): ${have}` +
    (ratings.imdb != null ? ` [imdb ${ratings.imdbIsExact ? "official dataset" : "OMDb snapshot"}]` : "")
  );

  // Pull any newly-seen IDs into the dataset for next time. Deliberately not
  // awaited -- a search must not wait on an 8 MB download.
...
async function getRatings(rawTitle, yearHint) {
  ...
  const promise = fetchRatings(normalized, year).catch((err) => {
