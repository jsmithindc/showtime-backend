...
// Poster artwork, as a fallback for films OMDb has none for. Free -- this page
// is already being fetched and parsed for the scores.
//
// Searched for rather than read from a fixed path: RT nests it differently
// across page variants, and a bounded scan tolerates that where a hardcoded
// path would silently return nothing. Same tactic as the runtime lookup in
// lib/movie-runtime.js, and for the same reason.
function findPosterUrl(node, depth = 0) {
  if (depth > 4 || !node || typeof node !== "object") return null;
  for (const [key, value] of Object.entries(node)) {
    if (typeof value === "string" &&
        /poster|image/i.test(key) &&
        /^https?:\/\/\S+\.(jpg|jpeg|png|webp)/i.test(value)) {
      return value;
    }
    if (value && typeof value === "object") {
      const found = findPosterUrl(value, depth + 1);
...
async function fetchScores(title, year) {
  // RT disambiguates same-titled films with a year suffix. When we have a year,
  // try the year-qualified slug FIRST -- a plain slug that hits an older
  // same-titled film would return valid scores and short-circuit without ever
  // trying the right one (e.g. resident_evil → 2002 film, not 2026).
  const base = toSlug(title);
  const slugs = year ? [`${base}_${year}`, base] : [base];
  ...
  for (const slug of slugs) {
    ...
    const poster = findPosterUrl(card);
    if (tomatometer == null && audience == null && !poster) continue;
