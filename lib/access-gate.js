 ...
 */
function createDailyBudget({ limit, warnAtFraction = 0.5, onWarn, onTrip } = {}) {
  if (!limit || limit <= 0) {
  ...
  // The EARLY signal is the one that's actually actionable. Tripping the
  // budget tells you abuse already happened; crossing half of it tells you
  // while there is still a day's headroom to react in -- which matters because
  // the intended reaction (setting ACCESS_KEY) takes a redeploy.
  const warnThreshold = Math.floor(limit * warnAtFraction);

  const mw = function dailyBudget(req, res, next) {
    const now = Date.now();
    if (now >= resetsAt) { used = 0; resetsAt = nextMidnight(now); warned = false; earlyWarned = false; }
    ...
    used += 1;
    // Once per day, not once per request over the line.
    if (!earlyWarned && warnThreshold > 0 && used >= warnThreshold) {
      earlyWarned = true;
      if (onWarn) onWarn({ used, limit, resetsAt });
    }
    next();
  };
  mw.usage = () => ({ used, limit, resetsAt, warnThreshold });
  return mw;
}
