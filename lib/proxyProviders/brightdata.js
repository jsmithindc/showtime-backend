...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  // schedule. See the comment in submitAndPollAsync for why.
  if (options.body) {
    console.error(`brightdata: using async path for ${targetUrl.split("?")[0]} (body present, method=${method})`);
    return submitAndPollAsync(targetUrl, options, apiKey, zone);
  }
  console.error(`brightdata: using sync path for ${targetUrl.split("?")[0]} (no body, method=${method})`);
  ...
  // Eagerly buffer the body so we can validate it before returning.
  // The sync endpoint has been observed returning status 200 with an
  // empty body (confirmed from a live "Unexpected end of JSON input"
  // error downstream). fetchWithRotation only falls through to the next
  // provider when fetchViaProvider throws -- if we return res directly,
  // the caller's res.json() throws AFTER rotation has already committed
  // to this provider, and apify-cfbypass never gets a chance. By
  // reading the body here and throwing on empty, we let fetchWithRotation's
  // per-provider catch trigger the fallthrough as intended.
  const bodyText = await res.text();
  if (!bodyText || !bodyText.trim()) {
    throw new Error(
      `brightdata sync: target returned HTTP ${res.status} but with an empty body -- ` +
      `falling through to the next provider.`
    );
  }
  if (bodyText.trimStart().startsWith("<")) {
    throw new Error(
      `brightdata sync: target returned HTML instead of JSON (status ${res.status}) -- ` +
      `falling through to the next provider.`
    );
  }
...
async function submitAndPollAsync(targetUrl, options, apiKey, zone) {
  ...
  // known to work.
  // Budget: 20s initial wait + one 5s retry = ~25s max before giving up.
  // Bright Data async jobs that succeed typically finish in the first check;
  // longer waits just block the rotation from reaching the next provider faster.
  // Restore to [20_000, 10_000, ...Array(6).fill(5_000)] if Bright Data
  // becomes the sole reliable provider for Regal's createOrder step.
  ...
  for (const delay of delays) {
    ...
    // If the body looks like HTML (Cloudflare challenge or error page), throw
    // so fetchWithRotation falls through to the next provider (apify-cfbypass).
    // Same pattern as the sync-path empty-body check: a 200 with an HTML body
    // is not a successful JSON API response, and the caller's res.json() would
    // throw AFTER rotation has already committed to this provider.
    if (bodyText && bodyText.trimStart().startsWith("<")) {
      throw new Error(
        `brightdata async: target returned HTML instead of JSON for ${targetUrl.split("?")[0]} ` +
        `(status ${targetStatus}) -- falling through to the next provider.`
      );
    }
  ...
  throw new Error(
    `brightdata: async result for response_id ${response_id} was not ready after ~${Math.round(delays.reduce((a, b) => a + b, 0) / 1000)}s of polling -- giving up ` +
    `(falling through to the next provider).`
  );
}
