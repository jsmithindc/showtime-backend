...
// Log all Bright Data response headers once so we can find the credit-cost
// header name. Runs only when the header hasn't been identified yet.
let _brightdataHeadersLogged = false;
function _logHeadersOnce(res) {
  if (_brightdataHeadersLogged) return;
  _brightdataHeadersLogged = true;
  const headers = {};
  res.headers.forEach((v, k) => { headers[k] = v; });
  console.error("brightdata: response headers (one-time log to find credit header):", JSON.stringify(headers));
}
