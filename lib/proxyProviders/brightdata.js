...
async function submitAndPollAsync(targetUrl, options, apiKey, zone) {
  ...
  // known to work.
  // Budget: 20s + 10s + 6×5s = ~60s max. Bounded lower than Bright Data's
  // own full schedule (~22 attempts / ~130s) because this provider sits first
  // in the rotation and a very long per-attempt wait compounds across the
  // retries in regal-scrapedo.js before apify-cfbypass ever gets a chance.
