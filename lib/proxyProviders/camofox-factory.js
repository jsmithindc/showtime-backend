...
function createCamofoxProvider(config) {
  ...
  async function acquireTab() {
    ...
    try {
      ...
      const rec = { id: openedId, expiresAt: Date.now() + TAB_TTL_MS, uses: 0 };
      liveTabCount++;
      startReaper();
  ...
  function releaseTab(rec, { discard = false } = {}) {
    // Retired on request count as well as TTL. Counted in REQUESTS, not
    // evaluates: a batch of 8 is 8 requests as far as the site is concerned,
    // and that is the thing being rationed.
    if (!discard && rec.uses >= MAX_REQUESTS_PER_TAB) {
      console.error(`${NAME}: retiring tab ${rec.id} after ${rec.uses} requests -- a fresh tab is a fresh session.`);
      discard = true;
    }
    if (discard) {
      liveTabCount--;
  ...
  // In-page concurrency. Regal's API returns small JSON and happily served ten
  // at once, but Cinemark's seat map is a full HTML page: firing ten in
  // parallel from one tab exhausted the browser's per-origin connection pool
  // and some came back "NetworkError when attempting to fetch resource" /
  // "The operation was aborted" -- 12 of ~19 priced. Limiting how many run at
  // once INSIDE the expression keeps the whole point of batching (still one
  // evaluate round trip for N urls) while not stampeding the origin.
  const BATCH_CONCURRENCY = config.batchConcurrency != null ? config.batchConcurrency : BATCH_MAX;
  // Gap between fetches within a lane, applied INSIDE the page. Lanes alone
  // weren't enough for Cinemark: two concurrent TicketSeatMap requests still
  // drew 429 challenges. This paces without costing extra round trips -- the
  // whole batch is still one evaluate.
  const BATCH_GAP_MS = config.batchGapMs != null ? config.batchGapMs : 0;
  // Retire a tab after this many requests. Cinemark's limit turned out to be a
  // SESSION quota rather than a rate: ~11 priced showings across three runs,
  // unmoved by a 400ms gap, with the first batch succeeding and the next
  // challenged. A fresh tab is a fresh session, so rotating addresses what is
  // actually being counted. Costs one tab open (~5s), which is why it is off
  // by default -- Regal serves 40+ on a single tab without complaint.
  const MAX_REQUESTS_PER_TAB = config.maxRequestsPerTab != null ? config.maxRequestsPerTab : Infinity;

  function buildBatchExpression(urls) {
    return `(async () => {
      const urls = ${JSON.stringify(urls)};
      const out = new Array(urls.length);
      const lanes = Math.min(${BATCH_CONCURRENCY}, urls.length);
      let next = 0;
      async function worker() {
        for (;;) {
          const i = next++;
          if (i >= urls.length) return;
          if (i >= lanes && ${BATCH_GAP_MS} > 0) await new Promise((r) => setTimeout(r, ${BATCH_GAP_MS}));
          try {
            const r = await fetch(urls[i], { method: "GET" });
            const text = await r.text();
            out[i] = { status: r.status, ok: r.ok, body: text };
          } catch (e) {
            // Recorded per URL rather than failing the whole batch: one bad
            // performance shouldn't cost the other nine their prices.
            out[i] = { status: 0, ok: false, body: String((e && e.message) || e) };
          }
        }
      }
