...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      // Cloudflare re-challenged this specific request from inside the browser.
      // This happens when Regal rate-limits concurrent createOrder POSTs.
      // Throw so fetchWithRotation falls through to Byparr/Bright Data.
      // Also invalidate the tab -- if CF is challenging API calls, the session
      // needs a fresh page load to clear it.
      if (body && body.includes("Just a moment...")) {
        console.error(
          `camofox-regal: Cloudflare challenge on ${targetUrl.split("?")[0]} ` +
          `(status ${status}) -- invalidating tab, falling through to next provider`
        );
        activeTabId = null;
        tabExpiresAt = 0;
