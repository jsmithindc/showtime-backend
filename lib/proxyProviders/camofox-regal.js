...
async function openRegalTab(attempt = 0) {
...
async function flushBatch() {
  ...
  try {
    ...
    if (!evalOk || !Array.isArray(result) || result.length !== urls.length) {
      throw new Error("camofox-regal: batch evaluate returned an unusable result");
    }
    releaseTab(rec);
    rec = null;
    console.error(`camofox-regal: batched ${urls.length} GETs into one evaluate.`);
    batch.items.forEach((item, i) => {
      ...
      if (body && body.includes("Just a moment...")) {
        ...
        item.reject(new Error(`camofox-regal: Cloudflare challenge (status ${status}) -- falling through`));
        return;
      }
    ...
    console.error(`camofox-regal: batch of ${urls.length} failed (${err.message}) -- falling back to single calls.`);
