...
//
// Was 10s while the app opened one cart PER SHOWTIME (8+ POSTs per search),
// where wide spacing was the only lever against Cloudflare's rate limit --
// and it never really worked. Since v5.8.3 shares one cart per theater,
// a search makes ~1 POST per Regal theater in range, which sits well inside
// the limit on its own. The gap is now just a courtesy stagger so two
// theaters don't fire simultaneously; 10s was costing ~9s of dead wait per
// extra theater for no benefit.
const POST_GAP_MS = 1000;
...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  // POST (createOrder): serialized so two theaters never fire at the same
  // instant. Gap tuning (1.5s -> 3s -> 5s -> 10s, v5.7.3 through v5.8.2) was a
  // dead end -- CF challenged after ~3 POSTs regardless of spacing, because
  // the real problem was VOLUME, not cadence: the app opened a cart per
  // showtime. v5.8.3 shares one cart per theater and the challenges stopped
  // entirely. Keep the queue, keep the gap small; don't reach for a bigger
  // gap if challenges reappear -- count the POSTs first.
  if (method === "POST") {
