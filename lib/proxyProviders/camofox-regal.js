...
async function fetchViaProvider(targetUrl, options = {}) {
  // createOrder (POST) is CF-challenged by Regal even from a cleared browser
  // session -- consistent after the first ~2 calls regardless of spacing.
  // Skip POSTs so fetchWithRotation falls through to Bright Data immediately,
  // without the queue overhead that was causing Bright Data async timeouts.
  // GET requests (getShowtimes, getTicketsForSession) work fine via evaluate.
  if ((options.method || "GET").toUpperCase() === "POST") {
    throw new Error(
      "camofox-regal: skipping POST (createOrder CF-challenged) -- falling through to direct HTTP providers"
    );
  }
