...
let lastPostAt = 0;
// CF allows ~3 createOrder calls per browser session before challenging.
// After POST_RESET_AFTER successes, invalidate the tab so the next call
// opens a fresh session with a clean CF window.
const POST_RESET_AFTER = 2;
...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  if (method === "POST") {
    const result = postQueueTail.then(async () => {
      ...
      lastPostAt = Date.now();
      const res = await doEvaluate(targetUrl, options);
      // Count successful POSTs and reset the tab session when we hit the CF
      // per-session limit (~3 calls). Fresh tab = fresh CF window.
      postSuccessCount++;
      if (postSuccessCount >= POST_RESET_AFTER) {
        console.error(`camofox-regal: resetting tab after ${postSuccessCount} successful createOrder(s) to avoid CF session limit`);
        activeTabId = null;
        tabExpiresAt = 0;
        postSuccessCount = 0;
      }
