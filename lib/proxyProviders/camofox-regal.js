...
// POST serialization: CF rate-limits rapid concurrent createOrder POSTs from
// the same browser session. Queue them with a minimum gap so each call fires
// after the previous one completes + POST_GAP_MS. First call goes immediately.
...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  // POST calls (createOrder) are serialized with a minimum gap between them
  // to avoid CF rate-limiting rapid concurrent POSTs from the same session.
  // Each call appends to the queue and waits its turn.
  if (method === "POST") {
...
// Inner implementation shared by GET and POST paths.
