...
//
// Byparr FIRST: self-hosted FlareSolverr-compatible Cloudflare bypass on a
// local LXC, exposed via Cloudflare Tunnel at byparr.flimcrickets.com. Free
// (no per-request credits), uses a persistent named browser session so the
// Cloudflare challenge is solved once and reused. Handles both GET and POST
// with body relay. Only active when BYPARR_URL + BYPARR_SECRET are set.
//
// Bright Data second: architecturally strong (explicit method/body relay in
// its own documented schema), real credentials confirmed working against
// Bright Data's own test endpoint. createOrder against Regal specifically has
// not been confirmed end-to-end -- still the best paid fallback if Byparr is
// unreachable.
//
// apify-cfbypass third: the one PAID provider confirmed, from a real captured
// response, to have cleared Regal's createOrder step end-to-end. Catches
// anything Byparr or Bright Data can't handle.
//
// Firecrawl fourth: still real and usable for GET-only steps, but cannot
...
const ALL_PROVIDERS = [byparr, brightdata, apifyCfBypass, firecrawl, zenrows, scrapedo];
...
async function fetchWithRotation(targetUrl, options = {}) {
  ...
  if (providers.length === 0) {
    throw new Error(
      "No proxy provider is configured -- set BYPARR_URL+BYPARR_SECRET, BRIGHTDATA_API_KEY+BRIGHTDATA_ZONE, APIFY_API_TOKEN, FIRECRAWL_API_KEY, ZENROWS_API_KEY, and/or SCRAPEDO_TOKEN in start.sh"
    );
  }
