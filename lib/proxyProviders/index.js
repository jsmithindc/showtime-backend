...
async function fetchWithRotation(targetUrl, options = {}) {
  ...
  for (const provider of providers) {
    // A provider may declare which URLs it can actually serve. camofox-regal
    // is the reason this exists: it executes fetch() inside a regmovies.com
    // browser tab, so any other host is a cross-origin request that the
    // browser rejects -- surfacing as `evaluate HTTP 500`. Landmark pricing
    // hit that on every call, burning most of its budget before a provider
    // that could do the job was even tried.
    if (typeof provider.supports === "function" && !provider.supports(targetUrl)) {
      skipped++;
      continue;
    }
    try {
  ...
  if (!lastError && skipped > 0) {
    throw new Error(
      `No configured proxy provider can fetch ${targetUrl.split("?")[0]} ` +
      `(${skipped} declined it as out of scope). Configure a general-purpose provider ` +
      `(BRIGHTDATA_API_KEY+BRIGHTDATA_ZONE, FIRECRAWL_API_KEY, ZENROWS_API_KEY, SCRAPEDO_TOKEN).`
    );
  }
  throw new Error(
