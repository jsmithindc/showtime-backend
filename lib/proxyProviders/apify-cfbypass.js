...
// Same constraint as camofox-regal: the actor loads regmovies.com and runs the
// request as a js_script inside that page (see `urls` in fetchViaProvider), so
// any other host is a cross-origin fetch the browser refuses. Declaring it
// here keeps non-Regal callers -- Landmark pricing in particular -- from
// burning their budget on a provider that cannot serve them.
...
module.exports = { NAME, isConfigured, fetchViaProvider, readCreditCost, looksLikeQuotaExceeded , supports };
