...
async function fetchViaProvider(targetUrl, options = {}) {
  ...
  let res = await callByparr(body);

  // 502/504/530 are transient Cloudflare Tunnel drops -- retry once before
  // falling through to the next provider.
  if ([502, 504, 530].includes(res.status)) {
    console.error(`byparr: transient ${res.status} -- retrying once`);
    await new Promise((r) => setTimeout(r, 1000));
    res = await callByparr(body);
  }
