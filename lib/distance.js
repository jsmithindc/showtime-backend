...
// Used only to decide which theaters are worth ASKING about, never to decide
// what the user sees. Deliberately optimistic: at 45mph nothing reachable in
// the stated time is filtered out before lib/drive-times.js gets a chance to
// price it properly. Over-including here is cheap (a few extra rows in one
// route-matrix call); under-including is the bug this whole change exists to
// fix, and is invisible when it happens.
const PREFILTER_SPEED_MPH = 45;
...
// Optimistic upper bound on what could be within `minutes` of the origin.
// Pair with a real drive-time lookup that re-filters the survivors; on its own
// this is far too generous to show anyone.
function prefilterMinutesAway(originLat, originLng, destLat, destLng) {
  const miles = milesBetween(originLat, originLng, destLat, destLng);
  return Math.round((miles / PREFILTER_SPEED_MPH) * 60);
}

module.exports = {
  milesBetween,
  estimatedMinutesAway,
  prefilterMinutesAway,
  minutesToMeters,
};
