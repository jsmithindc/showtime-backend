...
| **Brenden** | `priceAdapters/brenden-official.js` | Showtimes from the page, **pricing confirmed live** ($50.00 + $1.75 booking fee = $51.75 for The Odyssey 70mm; $15.75/$14.75/$14.75 for a standard screen -- both match their own checkout exactly). `robots.txt` disallows `/ahoy`, `/graphql` and `/checkout`, so **pricing runs on the operator's permission**, the same footing as Landmark; showtimes stay on the permitted `/{site}/showtimes` page, which is server-rendered (~19KB) and already carries every film, showtime and showing id. `/checkout` is still never fetched -- those URLs are buy links only. Runs on **Indy** (`indy.systems`), whose consumer GraphQL exposes **no price field at all** -- `Showing` offers only `priceCard { id name }` and `PriceCard` only `id`/`name`. The price is **assembled client-side** from three lists, which is why no single request carries it: `showingsForDate(date:)` (per-showing `priceCardId`, `showingBadgeIds`, and `seatsRemainingByTicketTypeId` -- the only signal for which ticket types a showing sells), `priceCards` (`priceCardItems[] = {ticketTypeId, hasCustomPrice, customPrice}`), and `clientConfig` (`ticketTypes` + `bookingFeeTypes`). Needs headers `site-id`, `circuit-id: 90`, `client-type: consumer` and **no cookies**; without `site-id` it answers "Site not found. (Code: 104)". Two requests per site per day cover every showing. **70mm is NOT a session attribute** -- Brenden lists the 70mm run as its own FILM ("The Odyssey - The IMAX 70mm Experience") beside a plain "The Odyssey", so the print is read off the title and the two must never be conflated; the API does state it outright as `displayMetaData.classes` `imax-70mm`, which `getShowingContext` picks up to confirm the title-derived flag. |
| **Apple Cinemas** | `priceAdapters/applecinemas-official.js` | Cinema360 ("C360OnlineSWeb") platform, runs with the operator's permission for personal use. **Routed through Camofox** (`camofox-applecinemas`) -- the API 403s with a Cloudflare challenge from any datacenter IP. Showtimes: `GET /Kiosk/GetLocationonlineMoviesOptimized/{companyId}/{movieId}/{from}/{to}`, whose `screenInfo` states `"IMAX 70MM"` outright. Pricing: `GET /PriceCard/GetKioskPriceCards/{locationId}/{priceCardId}` -- **parameter order is locationId FIRST**; `priceCard/location` 404s while `show/priceCard` and `company/priceCard` both return 200 with an EMPTY `tickets` array, a wrong-but-plausible answer that cost a round of guessing. Confirmed live: "70 MM IMAX Special Event" $26.00, plus a $1.00 Convenience Fee (per ticket, all channels) and a $1.75 Service Fee (per order, Web only). **`showTime` claims `+00:00` but is LOCAL time** -- trusting the offset shifts every showing four hours. **Known gap:** no location-scoped "what's playing here" endpoint found; `GetAllCompanyMoviesOptimized`, `GetLocationMoviesOptimized` and `GetAllCompanyLocationMoviesOptimized` all answer 200 with an empty array, so a movieId must come from elsewhere, as with `cinemark-movie-map.js`. |
| **Celebration!** | `priceAdapters/celebration-official.js` | Michigan regional, Vista platform. **Showtimes confirmed live; pricing not built yet.** Needs no API and no proxy: each `/cinemas/{slug}` page embeds its whole schedule as doubly-escaped JSON (entity-encoded into an attribute, quotes backslash-escaped inside that), and `robots.txt` is `Allow: /`. One GET gave 1137 sessions across several DAYS for GR North, 252 tagged `IMAX70mm` in `SessionAttributesNames` -- a STATED 70mm flag, unlike Atom (a Firecrawl credit per venue per day) or Harkins (only ever says "IMAX"). `StartTime` carries a real offset. All 11 locations are in `lib/celebration-cinemas.js`, regenerate with `node scripts/build-celebration-cinemas.js --write`. **Gotcha:** a cinema's Vista id is the dominant prefix of its PARSED SESSIONS, never of raw `NNN-NNNNNN` strings -- every page carries a fixed block of 66 cross-references to GR North's `002`, which outnumbered Getty Drive-In's own 61 sessions and mislabelled it as GR North. The pages are ~2.3MB and the site starts refusing connections after a few rapid fetches, hence one request per cinema per day through the shared cache. |
