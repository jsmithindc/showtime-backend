...
| **Brenden** | `priceAdapters/brenden-official.js` | **Showtimes only, by design.** `robots.txt` disallows `/ahoy`, `/graphql` and `/checkout` -- so `/{site}/showtimes` is permitted and their API is not, which settles the approach: parse the page, never call GraphQL. Nothing is lost by that: the page is server-rendered (~19KB) and already carries every film, showtime and showing id. **70mm is NOT a session attribute here** -- Brenden lists the 70mm run as its own FILM ("The Odyssey - The IMAX 70mm Experience") alongside a plain "The Odyssey" on another screen, both on the same page at 12:00/16:00/20:00 and 14:00. The print is read off the title and slug, and the two must never be conflated. **Pricing is deliberately absent:** every showtime links to `/{site}/checkout/showing/{slug}/{id}`, and `/checkout` is disallowed. Those URLs are offered as buy links -- linking is not fetching -- but the adapter never requests them, the same position already taken on Atom (whose `/checkout` pricing is why that path sits behind `ENABLE_ATOM_PATH`). |
